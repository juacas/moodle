...
class mod_feedback_complete_form extends moodleform {
    ...
    public function definition() {
        ...
        if (isloggedin() && !isguestuser() && $this->mode != self::MODE_EDIT && $this->mode != self::MODE_VIEW_TEMPLATE &&
            ...
            // Output information about the current mode (anonymous, trully anonymous or not) in some modes.
            switch ($this->structure->get_feedback()->anonymous) {
                case FEEDBACK_ANONYMOUS_TRULLY:
                    $anonymousmodeinfo = get_string('trully_anonymous', 'feedback');
                    break;
